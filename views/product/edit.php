...
<h3>SỬA SẢN PHẨM</h3>
<form action="?module=product&action=update&id=<?php echo $row['id'];?>" 
		...
		//	$c_rows = Query($sql);
			...
			while($c_row = mysqli_fetch_array($c_rows))
			{
				...
				<option value="<?php echo $c_row['id']?>" 
					<?php echo $c_row['id']== $row['category_id']?" selected ":"" ?>	
					>
					<?php echo $c_row['name'];?>
				</option>
				<?php 
		...
		//	$s_rows = Query($sql);
			...
			while($s_row = mysqli_fetch_array($s_rows))
			{
				...
				<option value="<?php echo $s_row['id']?>"
					<?php echo $s_row['id'] == $row['style_id']?" selected ":""?>
					>
					<?php echo $s_row['name'];?>
				</option>
				<?php 
	...
	<div class="form-group row">
		...
		<input class="form-control" type="text" name="size" id="size"  value="<?php echo $row['size']?>">
	</div>
	<div class="form-group row">
		<label for="weight" class="form-label">
			Trọng lượng
		</label>
		<input class="form-control" type="text" name="weight" id="weight"  value="<?php echo $row['weight']?>">
	</div>
	<div class="form-group row">
		...
		//	$p_rows = Query($sql);
			...
			while($p_row = mysqli_fetch_array($p_rows))
			{
				...
				<option value="<?php echo $p_row['id']?>"
					<?php echo $p_row['id']== $row['partner_id']?" selected":"";?>
					>
					<?php echo $p_row['name'];?>
				</option>
				<?php 
	...
	<div class="form-group row">
		...
		<input class="form-control" type="text" name="old_price" id="old_price"  value="<?php echo $row['old_price'];?>">
	</div>
	<div class="form-group row">
		...
		if(file_exists("uploads/{$row['id']}.jpg"))
			{?>
				<img src="uploads/<?php echo $row['id'];?>.jpg" style="width:25%;">
				<?php	
			}
			?>
